...
BOXES: dict[str, BoxSpec] = {
    ...
    "edge-s": BoxSpec("Smart Cam Monitoring Edge S (Intel N150)", inferences_per_sec=33.3,
                      ram_gb=16, shm_gb=4, cpu_cores=4),
    # Core Ultra: NPU + iGPU detector pool.
    "edge-m": BoxSpec("Smart Cam Monitoring Edge M (Core Ultra 5)", inferences_per_sec=90.0,
                      ram_gb=32, shm_gb=8, cpu_cores=12),
    # Same, plus a Hailo-8 M.2 (~7 ms YOLOv6n = ~143 inf/s on its own).
    "edge-m-hailo": BoxSpec("Smart Cam Monitoring Edge M + Hailo-8", inferences_per_sec=200.0,
                            ram_gb=32, shm_gb=8, cpu_cores=12),
}
