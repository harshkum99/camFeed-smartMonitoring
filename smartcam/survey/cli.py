"""`smartcam-survey` — read-only site survey.
...
each camera can actually support, and sizes the appliance.
    ...
    smartcam-survey scan   --cidr 192.168.1.0/24
    smartcam-survey probe  --host 192.168.1.64 --vendor hikvision --channels 16 --user admin
    smartcam-survey run    --cidr 192.168.1.0/24 --channels 16 --user admin --out survey

Credentials are read from a prompt or the SMARTCAM_CAM_PASSWORD environment variable, never from
...
from smartcam.survey.discovery import Device, discover, expand_cidr
from smartcam.survey.grade import ChannelGrade, ChannelProbe, grade_channel, size_box
from smartcam.survey.grammars import GRAMMARS, Vendor, candidates_for
from smartcam.survey.probe import ffprobe_available, probe_stream
from smartcam.survey.report import SurveyResult, now_iso, to_json, to_markdown


def _password(explicit: bool) -> str | None:
    env = os.environ.get("SMARTCAM_CAM_PASSWORD")
    if env:
        return env
    if explicit:
...
async def _probe_channels(
    ...
    *,
    ...
    exhaust a recorder's session limit and degrade the customer's own live view — and if we
...
def build_parser() -> argparse.ArgumentParser:
    ...
    def probing(sp: argparse.ArgumentParser) -> None:
        ...
        sp.add_argument("--ask-password", action="store_true",
                        help="prompt for the password (or set SMARTCAM_CAM_PASSWORD)")
